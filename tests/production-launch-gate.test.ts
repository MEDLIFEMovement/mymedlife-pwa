...
describe("production launch gate", () => {
  ...
  it("lists the staging and pilot launch evidence required before approval", () => {
    ...
    expect(
      gate.launchEvidenceChecks.find((check) => check.key === "luma_event_loop")
        ?.requiredEvidence,
    ).toContain("Proof rows as Ready");
    expect(
      gate.launchEvidenceChecks.find((check) => check.key === "luma_event_loop")
        ?.requiredEvidence,
    ).toContain("approved guest list");
    expect(
      gate.launchEvidenceChecks.find((check) => check.key === "luma_event_loop")
        ?.acceptanceSignal,
    ...
    expect(
      gate.launchEvidenceChecks.find((check) => check.key === "luma_event_loop")
        ?.acceptanceSignal,
    ).toContain("approved-guest evidence");
    expect(
      gate.launchEvidenceChecks.find((check) => check.key === "luma_event_loop")
        ?.acceptanceSignal,
    ).toContain("Proof rows Ready");
    expect(
      gate.launchEvidenceChecks.find((check) => check.key === "luma_event_loop")
        ?.reviewRoute,
    ...
    expect(
      gate.launchEvidenceChecks.find((check) => check.key === "luma_event_loop")
        ?.blockedUntil,
    ).toContain("signed-in reviewer session");
    expect(
      gate.launchEvidenceChecks.find((check) => check.key === "luma_event_loop")
        ?.blockedUntil,
    ).toContain("approved guest list");
  ...
  it("exposes the production Supabase and Vercel readiness packet without secrets", () => {
    ...
    expect(
      ...
      expect.arrayContaining([
        ...
        expect.stringContaining("/admin/theme"),
        expect.stringContaining("feature-flag save"),
        expect.stringContaining("theme-token save"),
      ]),
    );
    expect(
      gate.environmentReadiness.find(
      ...
      expect.arrayContaining([
        expect.stringContaining("default or env fallback posture"),
        expect.stringContaining("persistence warning"),
      ]),
    );
    expect(
