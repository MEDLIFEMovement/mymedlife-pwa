...
describe("chapter leader command center", () => {
  ...
  it("keeps chapter-home source context visible through event-loop routes", () => {
    ...
    expect(commandCenter.sourceContext).toMatchObject({
      ...
      title: "Opened from Chapter Home into attendance review",
      actions: [
        {
          label: "Back to Chapter Home",
          href: "/leader?view=overview&source=overview&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&quickAction=assign_action",
        },
        {
        ...
        },
        {
          ...
          href: "/leader?view=leaderboard&source=overview&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&quickAction=assign_action",
        },
      ],
    });
  ...
  it("keeps chapter-home leaderboard follow-through anchored to the event loop", () => {
    ...
    expect(commandCenter.sourceContext).toMatchObject({
      ...
      actions: [
        {
          label: "Back to Chapter Home",
          href: "/leader?view=overview&source=overview&member=member-ivy&eventCommittee=events&pipeline=follow_up&q=Ivy",
        },
        {
          label: "Open Event Performance",
  ...
  it("keeps chapter-home attendance context attached when the broader attendance lane opens", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "events",
      source: "overview",
    ...
    expect(markup).toContain(
      "href=\"/rush-month/actions?source=chapter_assign_action&amp;returnTo=%2Fleader%3Fview%3Devents%26source%3Doverview%26member%3Dmember-ivy%26eventCommittee%3Devents%26event%3Dbc-event-moving-mountains-kickoff%26pipeline%3Dfollow_up%26q%3DIvy%26quickAction%3Dassign_action&amp;member=member-ivy\"",
    );
  });

  it("keeps chapter-home attendance posture attached when leaderboard readback opens from attendance review", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "events",
      source: "overview",
    ...
    expect(markup).toContain(
      "href=\"/leader?view=leaderboard&amp;source=overview&amp;member=member-ivy&amp;eventCommittee=events&amp;event=bc-event-moving-mountains-kickoff&amp;leaderboardMetric=attendance&amp;pipeline=follow_up&amp;q=Ivy&amp;quickAction=assign_action\"",
    );
  });

  it("keeps chapter-home event detail anchored when leaderboard readback opens from the selected event", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "events",
      source: "overview",
    ...
    expect(markup).toContain(
      "href=\"/leader?view=leaderboard&amp;source=overview&amp;member=member-ivy&amp;eventCommittee=events&amp;event=bc-event-moving-mountains-kickoff&amp;leaderboardMetric=attendance&amp;pipeline=follow_up&amp;q=Ivy\"",
    );
  });
  ...
  it("keeps attendance-confirmation posture attached when member review opens from the leader event shell", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "events",
    ...
    expect(commandCenter.selectedMember?.backToContextLabel).toBe("Back to Attendance Review");
    expect(commandCenter.selectedMember?.backToContextHref).toBe(
      "/leader?view=events&source=events&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&quickAction=assign_action",
    );
    expect(commandCenter.selectedMember?.reviewContext).toMatchObject({
      eyebrow: "Event review follow-through",
      actionLabel: "Back to attendance review",
      actionHref:
        "/leader?view=events&source=events&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&quickAction=assign_action",
    });
  ...
  it("keeps chapter-home attendance follow-through attached when member review opens from overview", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "overview",
    ...
    expect(commandCenter.selectedMember?.backToContextLabel).toBe("Back to Chapter Home");
    expect(commandCenter.selectedMember?.backToContextHref).toBe(
      "/leader?view=overview&source=overview&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&pipeline=follow_up&q=Ivy&quickAction=assign_action",
    );
    expect(commandCenter.selectedMember?.reviewContext).toMatchObject({
      eyebrow: "Chapter home follow-through",
      actionLabel: "Back to Chapter Home",
      actionHref:
        "/leader?view=overview&source=overview&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&pipeline=follow_up&q=Ivy&quickAction=assign_action",
    });
    expect(markup).toContain("Chapter home follow-through");
    expect(markup).toContain("Back to Chapter Home");
    expect(markup).toContain(
  ...
  it("keeps chapter-home attendance context attached to blocked member-review actions", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "overview",
    ...
    expect(commandCenter.selectedMember?.leadershipActions[0]?.href).toBe(
      "/leader?view=member_profile&source=overview&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&pipeline=follow_up&q=Ivy&quickAction=promote_to_chair&returnQuickAction=assign_action",
    );
    expect(commandCenter.selectedMember?.leadershipActions[2]?.href).toBe(
      "/leader?view=member_profile&source=overview&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&pipeline=follow_up&q=Ivy&quickAction=assign_leadership_action&returnQuickAction=assign_action",
    );
  });

  it("keeps chapter-home attendance context attached to blocked interview and e-board follow-up actions", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "overview",
    ...
    expect(commandCenter.selectedMember?.leadershipActions[1]?.href).toBe(
      "/leader?view=member_profile&source=overview&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&pipeline=follow_up&q=Ivy&quickAction=schedule_values_interview&returnQuickAction=assign_action",
    );
    expect(commandCenter.selectedMember?.leadershipActions[3]?.href).toBe(
      "/leader?view=member_profile&source=overview&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&pipeline=follow_up&q=Ivy&quickAction=nominate_for_eboard&returnQuickAction=assign_action",
    );
  });

  it("keeps attendance-review context attached to blocked member-review actions from the leader event shell", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "events",
    ...
    expect(commandCenter.selectedMember?.leadershipActions[0]?.href).toBe(
      "/leader?view=member_profile&source=events&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&quickAction=promote_to_chair&returnQuickAction=assign_action",
    );
    expect(commandCenter.selectedMember?.leadershipActions[2]?.href).toBe(
      "/leader?view=member_profile&source=events&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&pipeline=follow_up&quickAction=assign_leadership_action&returnQuickAction=assign_action",
    );
  });

  it("keeps attendance-review context attached to blocked interview and e-board follow-up actions from the leader event shell", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "events",
    ...
    expect(commandCenter.selectedMember?.leadershipActions[1]?.href).toBe(
      "/leader?view=member_profile&source=events&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&quickAction=schedule_values_interview&returnQuickAction=assign_action",
    );
    expect(commandCenter.selectedMember?.leadershipActions[3]?.href).toBe(
      "/leader?view=member_profile&source=events&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&quickAction=nominate_for_eboard&returnQuickAction=assign_action",
    );
  });
  ...
  it("keeps event-owned leaderboard context attached when member review reopens leaderboard readback", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "leaderboard",
    ...
    expect(commandCenter.selectedMember?.backToContextLabel).toBe("Back to Leaderboard");
    expect(commandCenter.selectedMember?.backToContextHref).toBe(
      "/leader?view=leaderboard&source=leaderboard&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&region=canada&benchmark=leaderboard-mcgill",
    );
    expect(commandCenter.selectedMember?.reviewContext).toMatchObject({
      eyebrow: "Leaderboard follow-through",
      actionLabel: "Back to leaderboard",
  ...
  it("keeps attendance-review posture attached when member review reopens leaderboard readback", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "leaderboard",
    ...
    expect(commandCenter.selectedMember?.backToContextHref).toBe(
      "/leader?view=leaderboard&source=leaderboard&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&region=canada&quickAction=assign_action",
    );
    expect(commandCenter.selectedMember?.reviewContext).toMatchObject({
      actionLabel: "Back to leaderboard",
      actionHref:
  ...
  it("keeps benchmark attendance-review posture attached when member review reopens leaderboard readback", () => {
    ...
    expect(commandCenter.selectedMember?.backToContextHref).toBe(
      "/leader?view=leaderboard&source=leaderboard&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&region=canada&benchmark=leaderboard-mcgill&quickAction=assign_action",
    );
    expect(commandCenter.selectedMember?.reviewContext).toMatchObject({
      actionLabel: "Back to leaderboard",
      actionHref:
    ...
    expect(markup).toContain("Open attendance review");
    expect(markup).toContain(
      "href=\"/leader?view=leaderboard&amp;source=leaderboard&amp;member=member-ivy&amp;eventCommittee=events&amp;event=bc-event-moving-mountains-kickoff&amp;leaderboardMetric=attendance&amp;region=canada&amp;benchmark=leaderboard-mcgill&amp;quickAction=assign_action\"",
    );
    expect(markup).toContain(
      "href=\"/leader?view=events&amp;source=leaderboard&amp;member=member-ivy&amp;eventCommittee=events&amp;event=bc-event-moving-mountains-kickoff&amp;leaderboardMetric=attendance&amp;region=canada&amp;benchmark=leaderboard-mcgill&amp;quickAction=assign_action\"",
    );
  ...
  it("keeps attendance-review posture attached to blocked member-review actions from leaderboard readback", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "leaderboard",
    ...
    expect(commandCenter.selectedMember?.leadershipActions[0]?.href).toBe(
      "/leader?view=member_profile&source=leaderboard&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&region=canada&quickAction=promote_to_chair&returnQuickAction=assign_action",
    );
    expect(commandCenter.selectedMember?.leadershipActions[2]?.href).toBe(
      "/leader?view=member_profile&source=leaderboard&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&region=canada&pipeline=follow_up&quickAction=assign_leadership_action&returnQuickAction=assign_action",
    );
  });

  it("keeps attendance-review posture attached to blocked interview and e-board follow-up actions from leaderboard readback", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "leaderboard",
    ...
    expect(commandCenter.selectedMember?.leadershipActions[1]?.href).toBe(
      "/leader?view=member_profile&source=leaderboard&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&region=canada&quickAction=schedule_values_interview&returnQuickAction=assign_action",
    );
    expect(commandCenter.selectedMember?.leadershipActions[3]?.href).toBe(
      "/leader?view=member_profile&source=leaderboard&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&region=canada&quickAction=nominate_for_eboard&returnQuickAction=assign_action",
    );
  });

  it("keeps benchmark attendance-review posture attached to blocked interview and e-board follow-up actions from leaderboard readback", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "leaderboard",
    ...
    expect(commandCenter.selectedMember?.leadershipActions[1]?.href).toBe(
      "/leader?view=member_profile&source=leaderboard&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&region=canada&benchmark=leaderboard-mcgill&quickAction=schedule_values_interview&returnQuickAction=assign_action",
    );
    expect(commandCenter.selectedMember?.leadershipActions[3]?.href).toBe(
      "/leader?view=member_profile&source=leaderboard&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&region=canada&benchmark=leaderboard-mcgill&quickAction=nominate_for_eboard&returnQuickAction=assign_action",
    );
  });

  it("keeps attendance leaderboard context attached to blocked member-review actions", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "leaderboard",
  ...
  it("keeps chapter-home attendance-backed leaderboard posture visible before a member is selected", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = {
      ...getChapterLeaderCommandCenter(actor, data, {
        view: "leaderboard",
    ...
    expect(markup).toContain(
      "Opened from Chapter Home into attendance-backed leaderboard follow-through",
    );
    expect(markup).toContain("Back to Chapter Home");
    expect(markup).toContain("Open attendance review");
    expect(markup).toContain(
      "href=\"/leader?view=overview&amp;source=overview&amp;eventCommittee=events&amp;event=bc-event-moving-mountains-kickoff&amp;quickAction=assign_action\"",
    );
    expect(markup).toContain(
  ...
  it("keeps attendance-review context attached when leaderboard filters change inside event follow-through", () => {
    ...
    expect(commandCenter.leaderboardRegionOptions.find((option) => option.key === "all")).toMatchObject({
      href:
        "/leader?view=leaderboard&source=events&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&quickAction=assign_action",
    });
    expect(commandCenter.leaderboardRegionOptions.find((option) => option.key === "canada")).toMatchObject({
      href:
        "/leader?view=leaderboard&source=events&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&region=canada&quickAction=assign_action",
    });
    ...
    expect(commandCenter.leaderboardFilters.find((filter) => filter.key === "chapter_health")).toMatchObject({
      href:
        "/leader?view=leaderboard&source=events&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&region=canada&quickAction=assign_action",
    });
  });
  ...
  it("keeps attendance-review context attached across leader nav while review-members is active", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "members",
      source: "events",
    ...
    expect(commandCenter.viewOptions.find((item) => item.key === "overview")?.href).toBe(
      "/leader?view=overview&source=events&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&pipeline=follow_up&q=Ivy&quickAction=assign_action",
    );
    expect(commandCenter.viewOptions.find((item) => item.key === "leaderboard")?.href).toBe(
      "/leader?view=leaderboard&source=events&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&pipeline=follow_up&q=Ivy&quickAction=assign_action",
    );
  ...
  it("keeps chapter-home attendance context attached across leader nav while review-members is active", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "members",
      source: "overview",
    ...
    expect(commandCenter.viewOptions.find((item) => item.key === "leaderboard")?.href).toBe(
      "/leader?view=leaderboard&source=overview&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&pipeline=follow_up&q=Ivy&quickAction=assign_action",
    );
    expect(commandCenter.sourceContext?.actions).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
  ...
  it("keeps leaderboard attendance context attached across leader nav while review-members is active", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "members",
      source: "leaderboard",
    ...
    expect(commandCenter.viewOptions.find((item) => item.key === "overview")?.href).toBe(
      "/leader?view=overview&source=leaderboard&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&region=canada&pipeline=follow_up&q=Ivy&quickAction=assign_action",
    );
    expect(commandCenter.viewOptions.find((item) => item.key === "events")?.href).toBe(
      "/leader?view=events&source=leaderboard&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&region=canada&pipeline=follow_up&q=Ivy&quickAction=assign_action",
    );
    expect(commandCenter.sourceContext?.actions).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          label: "Back to leaderboard",
          href: "/leader?view=leaderboard&source=leaderboard&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&region=canada&pipeline=follow_up&q=Ivy&quickAction=assign_action",
        }),
        expect.objectContaining({
          label: "Open attendance review",
          href: "/leader?view=events&source=leaderboard&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&region=canada&pipeline=follow_up&q=Ivy&quickAction=assign_action",
        }),
      ]),
    );
  });

  it("keeps benchmark leaderboard context attached across leader nav while review-members is active", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "members",
      source: "leaderboard",
    ...
    expect(commandCenter.viewOptions.find((item) => item.key === "overview")?.href).toBe(
      "/leader?view=overview&source=leaderboard&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&region=canada&benchmark=leaderboard-mcgill&pipeline=follow_up&q=Ivy&quickAction=assign_action",
    );
    expect(commandCenter.viewOptions.find((item) => item.key === "events")?.href).toBe(
      "/leader?view=events&source=leaderboard&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&region=canada&benchmark=leaderboard-mcgill&pipeline=follow_up&q=Ivy&quickAction=assign_action",
    );
    expect(commandCenter.sourceContext?.actions).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          label: "Back to leaderboard",
          href: "/leader?view=leaderboard&source=leaderboard&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&region=canada&benchmark=leaderboard-mcgill&pipeline=follow_up&q=Ivy&quickAction=assign_action",
        }),
        expect.objectContaining({
          label: "Open attendance review",
          href: "/leader?view=events&source=leaderboard&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&region=canada&benchmark=leaderboard-mcgill&pipeline=follow_up&q=Ivy&quickAction=assign_action",
        }),
      ]),
    );
  });
  ...
  it("keeps attendance-review return posture visible while member review waits for a selected person", () => {
    ...
    expect(commandCenter.navigationMemberId).toBeNull();
    expect(commandCenter.viewOptions.find((item) => item.key === "leaderboard")?.href).toBe(
      "/leader?view=leaderboard&source=events&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&quickAction=assign_action",
    );
    expect(commandCenter.pipelineRows.find((row) => row.id === "member-ivy")?.profileHref).toBe(
      "/leader?view=member_profile&source=events&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&quickAction=assign_action",
    );
  ...
  it("keeps leaderboard readback return posture visible while member review waits for a selected person", () => {
    ...
    expect(commandCenter.navigationMemberId).toBeNull();
    expect(commandCenter.viewOptions.find((item) => item.key === "overview")?.href).toBe(
      "/leader?view=overview&source=leaderboard&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&region=canada&quickAction=assign_action",
    );
    expect(commandCenter.pipelineRows.find((row) => row.id === "member-ivy")?.profileHref).toBe(
      "/leader?view=member_profile&source=leaderboard&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&region=canada&quickAction=assign_action",
    );
  ...
  it("keeps leaderboard member-pipeline nav attached while member review waits for a selected person", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "leaderboard",
  ...
  it("keeps benchmark leaderboard readback return posture visible while member review waits for a selected person", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "leaderboard",
    ...
    expect(commandCenter.navigationMemberId).toBeNull();
    expect(commandCenter.viewOptions.find((item) => item.key === "overview")?.href).toBe(
      "/leader?view=overview&source=leaderboard&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&region=canada&benchmark=leaderboard-mcgill&quickAction=assign_action",
    );
    expect(commandCenter.pipelineRows.find((row) => row.id === "member-ivy")?.profileHref).toBe(
      "/leader?view=member_profile&source=leaderboard&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&region=canada&benchmark=leaderboard-mcgill&quickAction=assign_action",
    );
    expect(markup).toContain("Choose a member from leaderboard readback first");
    expect(markup).toContain("Back to leaderboard");
    expect(markup).toContain("Open attendance review");
  ...
  it("keeps benchmark leaderboard filter context attached while member review waits for a selected person", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "leaderboard",
  ...
  it("keeps chapter-home attendance return posture visible while member review waits for a selected person", () => {
    ...
    expect(commandCenter.navigationMemberId).toBeNull();
    expect(commandCenter.viewOptions.find((item) => item.key === "events")?.href).toBe(
      "/leader?view=events&source=overview&eventCommittee=events&event=bc-event-moving-mountains-kickoff&pipeline=follow_up&q=Ivy&quickAction=assign_action",
    );
    expect(commandCenter.pipelineRows[0]?.profileHref).toBe(
      "/leader?view=member_profile&source=overview&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&pipeline=follow_up&q=Ivy&quickAction=assign_action",
    );
  ...
  it("keeps chapter-home member-pipeline nav attached while member review waits for a selected person", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "overview",
  ...
  it("keeps attendance-review empty-state posture on direct load of blocked follow-through", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "events",
  ...
  it("keeps attendance-review empty-state posture on direct load of interview follow-through", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "events",
  ...
  it("keeps attendance-review member-pipeline nav on direct load of interview follow-through without a valid member", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "events",
  ...
  it("keeps chapter-home empty-state posture on direct load of e-board follow-through", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "overview",
  ...
  it("keeps chapter-home member-pipeline nav on direct load of e-board follow-through without a valid member", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "overview",
  ...
  it("keeps leaderboard attendance empty-state posture on direct load of interview follow-through", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "leaderboard",
  ...
  it("keeps leaderboard member-pipeline nav on direct load of e-board follow-through without a valid member", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "leaderboard",
  ...
  it("keeps benchmark leaderboard attendance empty-state posture on direct load of interview follow-through", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "leaderboard",
  ...
  it("keeps benchmark leaderboard member-pipeline nav on direct load of interview follow-through without a valid member", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "leaderboard",
  ...
  it("keeps benchmark leaderboard attendance empty-state posture on direct load of e-board follow-through", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "leaderboard",
  ...
  it("keeps benchmark leaderboard member-pipeline nav on direct load of e-board follow-through without a valid member", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "leaderboard",
  ...
  it("keeps attendance-review empty-state posture on direct load of open-event-context follow-through", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "events",
  ...
  it("keeps attendance-review member-pipeline nav on direct load of open-event-context follow-through without a valid member", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "events",
  ...
  it("keeps chapter-home attendance empty-state posture on direct load of open-event-context follow-through", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "overview",
  ...
  it("keeps chapter-home member-pipeline nav on direct load of open-event-context follow-through without a valid member", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "overview",
  ...
  it("keeps leaderboard attendance empty-state posture on direct load of open-event-context follow-through", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "leaderboard",
  ...
  it("keeps leaderboard member-pipeline nav on direct load of open-event-context follow-through without a valid member", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "leaderboard",
  ...
  it("keeps benchmark leaderboard attendance empty-state posture on direct load of open-event-context follow-through", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "leaderboard",
  ...
  it("keeps benchmark leaderboard member-pipeline nav on direct load of open-event-context follow-through without a valid member", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "leaderboard",
  ...
  it("keeps benchmark leaderboard attendance empty-state posture on direct load of blocked follow-through", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "leaderboard",
  ...
  it("keeps quick actions context-only while event-shell member review waits for a valid selected person", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "events",
    ...
    expect(commandCenter.navigationMemberId).toBeNull();
    expect(commandCenter.quickActions.find((action) => action.label === "Confirm Attendance")?.href).toBe(
      "/leader?view=events&source=events&eventCommittee=events&event=bc-event-moving-mountains-kickoff&quickAction=assign_action",
    );
    expect(commandCenter.quickActions.find((action) => action.label === "Review Members")?.href).toBe(
      "/leader?view=members&source=events&eventCommittee=events&event=bc-event-moving-mountains-kickoff&quickAction=review_members",
    );
    expect(commandCenter.quickActions.find((action) => action.label === "Review Leaderboard")?.href).toBe(
      "/leader?view=leaderboard&source=events&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&quickAction=assign_action",
    );
  ...
  it("keeps quick actions context-only while chapter-home member review waits for a valid selected person", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "overview",
    ...
    expect(commandCenter.quickActions.find((action) => action.label === "Review Members")?.href).toBe(
      "/leader?view=members&source=overview&eventCommittee=events&event=bc-event-moving-mountains-kickoff&pipeline=follow_up&q=Ivy&quickAction=review_members",
    );
    expect(commandCenter.quickActions.find((action) => action.label === "Review Leaderboard")?.href).toBe(
      "/leader?view=leaderboard&source=overview&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&pipeline=follow_up&q=Ivy&quickAction=assign_action",
    );
  ...
  it("keeps attendance-review return context visible while add-note is open from the leader event shell", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "events",
  ...
  it("keeps chapter-home attendance return context visible while add-note is open from chapter home", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "overview",
  ...
  it("keeps leaderboard attendance return context visible while add-note is open from leaderboard follow-through", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "leaderboard",
    ...
    expect(markup).toContain("Return to profile");
    expect(markup).toContain(
      "href=\"/leader?view=member_profile&amp;source=leaderboard&amp;member=member-ivy&amp;eventCommittee=events&amp;event=bc-event-moving-mountains-kickoff&amp;leaderboardMetric=attendance&amp;region=canada&amp;returnQuickAction=assign_action\"",
    );
  });

  it("keeps attendance-review context attached across leader nav while add-note is open from event review", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "events",
    ...
    expect(commandCenter.viewOptions.find((item) => item.key === "overview")?.href).toBe(
      "/leader?view=overview&source=events&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&quickAction=assign_action",
    );
    expect(commandCenter.viewOptions.find((item) => item.key === "leaderboard")?.href).toBe(
      "/leader?view=leaderboard&source=events&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&quickAction=assign_action",
    );
  ...
  it("keeps chapter-home attendance context attached across leader nav while add-note is open", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "overview",
  ...
  it("keeps leaderboard attendance context attached across leader nav while add-note is open", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "leaderboard",
    ...
    expect(commandCenter.viewOptions.find((item) => item.key === "overview")?.href).toBe(
      "/leader?view=overview&source=leaderboard&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&region=canada&quickAction=assign_action",
    );
    expect(commandCenter.viewOptions.find((item) => item.key === "events")?.href).toBe(
      "/leader?view=events&source=leaderboard&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&region=canada&quickAction=assign_action",
    );
    expect(commandCenter.sourceContext?.actions).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
  ...
  it("keeps benchmark leaderboard attendance context attached across leader nav while add-note is open", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "leaderboard",
    ...
    expect(commandCenter.viewOptions.find((item) => item.key === "overview")?.href).toBe(
      "/leader?view=overview&source=leaderboard&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&region=canada&benchmark=leaderboard-mcgill&quickAction=assign_action",
    );
    expect(commandCenter.viewOptions.find((item) => item.key === "events")?.href).toBe(
      "/leader?view=events&source=leaderboard&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&region=canada&benchmark=leaderboard-mcgill&quickAction=assign_action",
    );
    expect(commandCenter.sourceContext?.actions).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
  ...
  it("keeps attendance-review empty-state posture when add-note reloads without a valid member", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "events",
  ...
  it("keeps attendance-review member-pipeline nav when add-note reloads without a valid member", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "events",
  ...
  it("keeps chapter-home attendance empty-state posture when add-note reloads without a valid member", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "overview",
  ...
  it("keeps chapter-home member-pipeline nav when add-note reloads without a valid member", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "overview",
  ...
  it("keeps leaderboard attendance empty-state posture when add-note reloads without a valid member", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "leaderboard",
  ...
  it("keeps leaderboard member-pipeline nav when add-note reloads without a valid member", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "leaderboard",
  ...
  it("keeps benchmark leaderboard attendance empty-state posture when add-note reloads without a valid member", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "leaderboard",
  ...
  it("keeps benchmark leaderboard member-pipeline nav when add-note reloads without a valid member", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "leaderboard",
  ...
  it("keeps attendance-review return context visible while chair review is open from attendance review", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "events",
  ...
  it("keeps attendance-review context on the member-pipeline return path from event review", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "events",
    ...
    expect(commandCenter.selectedMember?.backToPipelineHref).toBe(
      "/leader?view=members&source=events&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&returnQuickAction=assign_action",
    );
    expect(markup).toContain(
      "href=\"/leader?view=members&amp;source=events&amp;member=member-ivy&amp;eventCommittee=events&amp;event=bc-event-moving-mountains-kickoff&amp;returnQuickAction=assign_action\"",
    );
  });
  ...
  it("keeps chapter-home attendance context on the member-pipeline return path", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "overview",
    ...
    expect(commandCenter.selectedMember?.backToPipelineHref).toBe(
      "/leader?view=members&source=overview&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&pipeline=follow_up&q=Ivy&returnQuickAction=assign_action",
    );
    expect(markup).toContain(
      "href=\"/leader?view=members&amp;source=overview&amp;member=member-ivy&amp;eventCommittee=events&amp;event=bc-event-moving-mountains-kickoff&amp;pipeline=follow_up&amp;q=Ivy&amp;returnQuickAction=assign_action\"",
    );
  });

  it("keeps attendance-review context attached across leader nav from blocked event follow-through", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "events",
  ...
  it("keeps chapter-home attendance context attached across leader nav from blocked follow-through", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "overview",
  ...
  it("keeps chapter-home attendance context when open-event-context is loaded from member review", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "overview",
    ...
    expect(markup).toContain("Open Event Performance");
    expect(markup).toContain(
      "href=\"/leader?view=events&amp;source=overview&amp;member=member-ivy&amp;eventCommittee=events&amp;event=bc-event-moving-mountains-kickoff&amp;pipeline=follow_up&amp;q=Ivy&amp;quickAction=assign_action\"",
    );
  });

  it("keeps leaderboard attendance context when open-event-context is loaded from member review", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "leaderboard",
    ...
    expect(markup).toContain("Open Event Performance");
    expect(markup).toContain(
      "href=\"/leader?view=events&amp;source=leaderboard&amp;member=member-ivy&amp;eventCommittee=events&amp;event=bc-event-moving-mountains-kickoff&amp;leaderboardMetric=attendance&amp;region=canada&amp;benchmark=leaderboard-mcgill&amp;pipeline=follow_up&amp;quickAction=assign_action\"",
    );
  });

  it("keeps attendance-review context attached across leader nav while open-event-context is active", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "events",
    ...
    expect(commandCenter.viewOptions.find((item) => item.key === "overview")?.href).toBe(
      "/leader?view=overview&source=events&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&pipeline=follow_up&quickAction=assign_action",
    );
    expect(commandCenter.viewOptions.find((item) => item.key === "leaderboard")?.href).toBe(
      "/leader?view=leaderboard&source=events&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&pipeline=follow_up&quickAction=assign_action",
    );
    expect(commandCenter.sourceContext?.actions).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
  ...
  it("keeps chapter-home attendance context attached across leader nav while open-event-context is active", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "overview",
  ...
  it("keeps leaderboard attendance context attached across leader nav while open-event-context is active", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "leaderboard",
    ...
    expect(commandCenter.viewOptions.find((item) => item.key === "overview")?.href).toBe(
      "/leader?view=overview&source=leaderboard&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&region=canada&pipeline=follow_up&quickAction=assign_action",
    );
    expect(commandCenter.viewOptions.find((item) => item.key === "events")?.href).toBe(
      "/leader?view=events&source=leaderboard&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&region=canada&pipeline=follow_up&quickAction=assign_action",
    );
    expect(commandCenter.sourceContext?.actions).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          label: "Back to leaderboard",
          href: "/leader?view=leaderboard&source=leaderboard&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&region=canada&pipeline=follow_up&quickAction=assign_action",
        }),
        expect.objectContaining({
          label: "Open attendance review",
          href: "/leader?view=events&source=leaderboard&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&region=canada&pipeline=follow_up&quickAction=assign_action",
        }),
      ]),
    );
  });

  it("keeps benchmark leaderboard context attached across leader nav while open-event-context is active", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "leaderboard",
    ...
    expect(commandCenter.viewOptions.find((item) => item.key === "overview")?.href).toBe(
      "/leader?view=overview&source=leaderboard&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&region=canada&benchmark=leaderboard-mcgill&pipeline=follow_up&quickAction=assign_action",
    );
    expect(commandCenter.viewOptions.find((item) => item.key === "events")?.href).toBe(
      "/leader?view=events&source=leaderboard&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&region=canada&benchmark=leaderboard-mcgill&pipeline=follow_up&quickAction=assign_action",
    );
    expect(commandCenter.sourceContext?.actions).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          label: "Back to leaderboard",
          href: "/leader?view=leaderboard&source=leaderboard&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&region=canada&benchmark=leaderboard-mcgill&pipeline=follow_up&quickAction=assign_action",
        }),
        expect.objectContaining({
          label: "Open attendance review",
          href: "/leader?view=events&source=leaderboard&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&region=canada&benchmark=leaderboard-mcgill&pipeline=follow_up&quickAction=assign_action",
  ...
  it("keeps leaderboard attendance return context visible while chair review is open from attendance review", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "leaderboard",
    ...
    expect(markup).toContain(
      "href=\"/rush-month/events/bc-event-moving-mountains-kickoff?source=chapter_event_review&amp;returnTo=%2Fleader%3Fview%3Devents%26source%3Dleaderboard%26member%3Dmember-ivy%26eventCommittee%3Devents%26event%3Dbc-event-moving-mountains-kickoff%26quickAction%3Dassign_action\"",
    );
    expect(markup).toContain(
      "href=\"/leader?view=leaderboard&amp;source=leaderboard&amp;member=member-ivy&amp;eventCommittee=events&amp;event=bc-event-moving-mountains-kickoff&amp;leaderboardMetric=attendance&amp;region=canada&amp;quickAction=assign_action\"",
    );
  ...
  it("keeps benchmark leaderboard return context visible on direct load of blocked follow-through", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "leaderboard",
  ...
  it("keeps leaderboard attendance context on the member-pipeline return path", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "leaderboard",
    ...
    expect(commandCenter.selectedMember?.backToPipelineHref).toBe(
      "/leader?view=members&source=leaderboard&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&region=canada&returnQuickAction=assign_action",
    );
    expect(markup).toContain(
      "href=\"/leader?view=members&amp;source=leaderboard&amp;member=member-ivy&amp;eventCommittee=events&amp;event=bc-event-moving-mountains-kickoff&amp;leaderboardMetric=attendance&amp;region=canada&amp;returnQuickAction=assign_action\"",
    );
  });

  it("keeps attendance-backed leaderboard context attached across leader nav from blocked member review", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "leaderboard",
    ...
    expect(commandCenter.leaderboardRegionOptions.find((option) => option.key === "all")).toMatchObject({
      href:
        "/leader?view=leaderboard&source=leaderboard&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&quickAction=assign_action",
    });
    expect(commandCenter.leaderboardFilters.find((filter) => filter.key === "chapter_health")).toMatchObject({
      href:
        "/leader?view=leaderboard&source=leaderboard&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&region=canada&quickAction=assign_action",
    });
  });

  it("keeps benchmark leaderboard context attached across leader nav from blocked member review", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "member_profile",
      source: "leaderboard",
  ...
  it("opens best practices as a leaderboard-owned handoff into feed analytics", () => {
    ...
    expect(commandCenter.sourceContext?.actions?.[0]).toMatchObject({
      label: "Back to leaderboard",
      href:
        "/leader?view=leaderboard&source=leaderboard&leaderboardMetric=attendance&benchmark=leaderboard-ucla",
    });
  ...
  it("keeps attendance-backed leaderboard context attached across command-center nav", () => {
    const actor = getMockLocalActorContext("leader.a@mymedlife.test");
    const commandCenter = getChapterLeaderCommandCenter(actor, data, {
      view: "leaderboard",
      source: "leaderboard",
    ...
    expect(commandCenter.viewOptions.find((item) => item.key === "members")?.href).toBe(
      "/leader?view=members&source=leaderboard&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&region=canada&quickAction=assign_action",
    );
    expect(commandCenter.viewOptions.find((item) => item.key === "events")?.href).toBe(
      "/leader?view=events&source=leaderboard&member=member-ivy&eventCommittee=events&event=bc-event-moving-mountains-kickoff&leaderboardMetric=attendance&region=canada&quickAction=assign_action",
    );
...
describe("chapter leader command center href", () => {
  ...
  it("builds the broader assignment lane href with chapter return context preserved", () => {
    expect(
      ...
      "/rush-month/actions?source=chapter_assign_action&returnTo=%2Fleader%3Fview%3Devents%26source%3Doverview%26member%3Dmember-zara%26eventCommittee%3Devents%26event%3Dbc-event-moving-mountains-kickoff%26pipeline%3Dfollow_up%26q%3DIvy%26quickAction%3Dassign_action&member=member-zara",
    );
    expect(
      buildChapterLeaderAssignmentFlowHref({
        memberId: "member-zara",
