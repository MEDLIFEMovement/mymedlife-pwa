...
describe("staff page", () => {
  ...
  it("renders populated live and mock staff readbacks with truthful shared source labels", () => {
    const organization = {
      totalPoints: 125,
      totalRsvps: 4,
      totalAttendance: 3,
      chaptersWithPoints: 1,
      topChapterName: "UCLA MEDLIFE",
      topChapterPoints: 125,
      featuredEventTitle: "TEST kickoff",
      featuredEventChapterName: "UCLA MEDLIFE",
      featuredEventAttendanceCount: 3,
      featuredEventPointsAwarded: 125,
    };
    const chapters = [{
      id: "chapter-ucla",
      name: "UCLA MEDLIFE",
      chapterEventId: "event-ucla",
      detailHref: "/staff?view=events&event=event-ucla",
      calendarLabel: "App-owned calendar",
      calendarStatusLabel: "Ready",
      calendarReady: true,
      nextEvent: "TEST kickoff",
      rsvps: 4,
      attendance: 3,
      points: 125,
    ...
    const liveSource = {
      ...
      message: "Reading current app-owned rows.",
    };
    const mockSource = {
      mode: "mock" as const,
      status: "mock_fallback" as const,
      message: "Using TEST fixture rows.",
    };
    ...
    expect(liveHtml).toContain("Supabase operational truth");
    expect(liveHtml).toContain("UCLA MEDLIFE");
    expect(liveHtml).not.toContain("TEST UCLA MEDLIFE");
    ...
    expect(mockHtml).toContain("TEST / unavailable readback");
    expect(mockHtml).toContain("TEST UCLA MEDLIFE");
    expect(mockHtml).toContain("TEST kickoff");
    expect(mockHtml).not.toContain("TEST TEST kickoff");

    const chapterWithoutEvent = {
      ...chapters[0],
      chapterEventId: null,
      detailHref: null,
      nextEvent: "No upcoming event",
      risk: "No upcoming events" as const,
    };
    const organizationWithoutPoints = {
      ...organization,
      topChapterName: null,
      topChapterPoints: 0,
    };
    const unavailableHtml = [
      renderToStaticMarkup(
        <StaffLiveChapterPortfolio
          chapters={[chapterWithoutEvent]}
          organization={organizationWithoutPoints}
          source={liveSource}
        />,
      ),
      renderToStaticMarkup(
    ...
    expect(unavailableHtml).toContain("Selected event is not available in the staff readback.");
    expect(unavailableHtml).toContain("No event");
    expect(unavailableHtml).toContain("Pending");
    expect(unavailableHtml).toContain("No points yet");
  });
