...
const usage = [
  "Usage:",
  "  pnpm rollout:owner-requests --owner-dir rollout-owner-packets --out production-rollout-owner-requests [--minimum-chapters=30] [--minimum-students=500] [--minimum-pilot-chapters=5]",
  "",
  "This reads owner-specific rollout CSV folders and writes owner-by-owner request docs.",
...
try {
  ...
  const [
    ...
    import("../src/services/production-rollout-owner-requests.ts"),
  ...
  for (const file of getProductionRolloutOwnerRequestFiles(status)) {
    await writeFile(join(outDir, file.path), file.content);
  }

  console.log(`Production rollout owner request package written to ${outDir}`);
  ...
  console.error("Production rollout owner request package was not created.");
  console.error("");
