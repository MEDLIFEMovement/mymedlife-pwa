...
export async function runReleaseProvenanceCheck({
  ...
  const [baseUrlInput = "https://mymedlife.org", expectedShaInput] = args;
  ...
  try {
    ...
    const expectedSha = normalizeSha(
      expectedShaInput ?? env.EXPECTED_RELEASE_SHA,
    );
    if (!expectedSha) {
      throw new Error("An exact 40-character expected commit SHA is required.");
    }

    const response = await fetchImpl(`${baseUrl}/api/release`, {
      cache: "no-store",
      redirect: "error",
    });
    ...
    log(`Production release provenance: ${ready ? "READY" : "NOT READY"}`);
    log(`Base URL: ${baseUrl}`);
    log(`Expected commit: ${expectedSha}`);
    log("");
    for (const check of checks) {
      log(`${check.passed ? "PASS" : "FAIL"} ${check.label}: ${check.detail}`);
    }
    return ready ? 0 : 1;
  } catch (error) {
    logError("Production release provenance: NOT READY");
    logError("");
