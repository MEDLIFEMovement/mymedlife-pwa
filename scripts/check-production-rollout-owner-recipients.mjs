...
const usage = [
  "Usage:",
  "  pnpm rollout:owner-recipients --owner-dir rollout-owner-packets --recipient-assignments production-rollout-owner-send-tracker/owner-recipient-assignments.csv --out production-rollout-owner-recipient-status.md [--minimum-chapters=30] [--minimum-students=500] [--minimum-pilot-chapters=5]",
  "",
  "This checks whether every owner packet has a valid recipient before manual sendout.",
...
try {
  ...
  const recipientAssignmentsPath = resolve(args.recipientAssignments);
  ...
  const ownerStatus = getProductionRolloutOwnerPacketStatus({
    foundFiles,
  ...
  const recipientStatus = getProductionRolloutOwnerRecipientStatus({
    ...
    recipientAssignments: parseProductionRolloutOwnerRecipientAssignmentsCsv(
      await readFile(recipientAssignmentsPath, "utf8"),
    ),
  });

  await mkdir(dirname(outPath), { recursive: true });
  await writeFile(outPath, formatProductionRolloutOwnerRecipientStatus(recipientStatus));

  console.log(`Production rollout owner recipient status written to ${outPath}`);
  console.log(`Current status: ${recipientStatus.readyForOwnerPacketSend ? "READY TO SEND" : "NOT READY"}`);
  console.log(`Owner recipients assigned: ${recipientStatus.summary.assignedOwnerCount}/${recipientStatus.summary.ownerCount}`);
  console.log(`Recipient issues: ${recipientStatus.summary.issueCount}`);

  if (!recipientStatus.readyForOwnerPacketSend) {
    process.exit(1);
  }
} catch (error) {
  console.error("Production rollout owner recipient status was not created.");
  console.error("");
