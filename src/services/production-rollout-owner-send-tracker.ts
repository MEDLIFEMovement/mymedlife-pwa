...
export function formatProductionRolloutOwnerSendTrackerReadme({
  ...
  const lines = [
    ...
    "## Files",
    "",
    "- `owner-send-tracker.csv`: one row per owner packet, with blank columns for recipient, draft evidence, and send/return dates.",
    ...
    "2. If a Gmail draft exists, fill `draftId` and `draftedAt` so the handoff can prove draft evidence separately from send evidence.",
    "3. Set `sendStatus` to `drafted`, `sent`, `returned`, or `validated` as the owner handoff moves.",
    "4. Fill `sentAt`, `returnedAt`, and `validatedAt` with ISO dates when each step happens.",
    "5. Keep owner CSV headers unchanged when returned files are copied back.",
    "6. Rerun owner status and current status after every returned owner folder.",
    "7. Optional: fill `owner-recipient-assignments.csv`, then rerun the tracker command with `--recipient-assignments owner-recipient-assignments.csv` to prefill recipients.",
    "8. Before sending owner packets, run `pnpm rollout:owner-recipient-decisions --owner-dir <owner-dir> --recipient-assignments owner-recipient-assignments.csv --out production-rollout-owner-recipient-decisions.md`.",
    "9. Then run `pnpm rollout:owner-recipients --owner-dir <owner-dir> --recipient-assignments owner-recipient-assignments.csv --out production-rollout-owner-recipient-status.md`.",
    "",
    "## Status Values",
    "",
