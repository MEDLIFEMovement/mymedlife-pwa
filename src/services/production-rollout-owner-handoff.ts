...
function formatProductionRolloutOwnerHandoffIndex({
  ...
  return [
    ...
    "## Send Order",
    "",
    "1. Use the owner-recipient decision worksheet to collect the Copy/Paste Answer Block in `owner-recipient-answers.txt`.",
    "2. Run `pnpm rollout:owner-recipient-answers` to turn that answer block into `production-rollout-owner-send-tracker/owner-recipient-assignments.csv`.",
    "3. Regenerate the send tracker with `--recipient-assignments` if you want recipient columns prefilled.",
    "4. Send each owner their matching folder under `rollout-owner-packets/`.",
    "5. Use each owner's matching email draft under `production-rollout-owner-email-drafts/`.",
    "6. Track sent/returned/validated status in `production-rollout-owner-send-tracker/owner-send-tracker.csv`.",
    "7. Include each owner's matching Markdown request under `production-rollout-owner-requests/`.",
    "8. Ask each owner to return only their completed CSV files.",
    "9. Rebuild this status before assembling the shared rollout CSV folder.",
    "10. Do not invite students until the final invite gate passes.",
    "",
    ...
    `pnpm rollout:owner-recipient-answers --answers owner-recipient-answers.txt --owner-dir ${outputDirectoryName}/${ownerDirectoryName} --out ${outputDirectoryName}/${sendTrackerDirectoryName}/owner-recipient-assignments.csv`,
    ...
    `pnpm rollout:owner-return-intake --returns-dir returned-owner-packets --owner-dir ${outputDirectoryName}/${ownerDirectoryName} --out production-rollout-owner-return-intake.md`,
    `pnpm rollout:owner-return-intake --returns-dir returned-owner-packets --owner-dir ${outputDirectoryName}/${ownerDirectoryName} --out production-rollout-owner-return-intake.md --apply`,
