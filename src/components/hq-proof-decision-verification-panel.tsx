    ...
    <section className="app-surface-info rounded-[2rem] p-5">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-semibold text-slate-950">{packet.title}</h1>
          <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600">{packet.plainEnglishSummary}</p>
      ...
      {packet.candidateEvidence ? (
        <article className="app-surface mt-5 rounded-[1.5rem] p-4">
        ...
        <article className="app-surface-warm mt-5 rounded-[1.5rem] p-4">
      ...
      <section className="app-surface mt-5 rounded-[2rem] p-4">
        ...
        <div className="mt-4 grid gap-3 lg:grid-cols-[1fr_0.9fr]">
          ...
          <article className="app-surface-soft rounded-[1.4rem] p-4">
            ...
            <p className="app-surface-warm mt-3 rounded-[1.05rem] p-3 text-xs leading-5 text-slate-600">
          ...
          {packet.verificationPacket.operatorSequence.map((step, index) => (
            ...
            >
              <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                ...
                <Link
                  href={step.route}
                  className="rounded-full border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-500"
                >
                  Open {step.route}
                </Link>
              </div>
        ...
        <article className="app-surface-warm mt-4 rounded-[1.4rem] p-4">
          <p className="text-sm font-semibold text-slate-950">Stop conditions</p>
          <ul className="mt-3 grid gap-2">
            {packet.verificationPacket.safetyStops.map((stop) => (
              <li key={stop} className="text-xs leading-5 text-slate-600">
                {stop}
              </li>
            ))}
          </ul>
      ...
      <div className="mt-5 grid gap-3 lg:grid-cols-2">
        {packet.checks.map((check) => (
          <article
            key={check.key}
            className="app-surface rounded-[1.4rem] p-4"
          >
            <p
              className={
                check.passed
            ...
            <h2 className="mt-2 text-lg font-semibold text-slate-950">{check.label}</h2>
