  ...
  return (
    <section className="rounded-[2rem] border border-white/12 bg-white/[0.05] p-5">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        ...
        <span className="rounded-full border border-white/10 bg-[var(--mymedlife-border)]/40 px-3 py-1 text-xs font-semibold text-white/62">
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-2">
        {glossary.terms.map((item) => (
          <article key={item.term} className="rounded-2xl border border-white/10 bg-[var(--mymedlife-border)]/40 p-4">
