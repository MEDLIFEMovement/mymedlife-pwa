...
function ProofDecisionCard({ row }: { row: LeaderProofDecisionRow }) {
  return (
    <article className="rounded-2xl border border-white/10 bg-[var(--mymedlife-border)]/40 p-4">
      ...
      <div className="mt-4 rounded-2xl border border-white/10 bg-[var(--mymedlife-admin-blue)]/70 p-3">
        ...
        <div className="mt-3 grid gap-2 md:grid-cols-2">
          {row.reviewRubric.map((item) => (
            <div key={`${row.key}-${item.label}`} className="rounded-2xl bg-[var(--mymedlife-border)]/40 p-3">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {row.decisionOptions.map((option) => (
            <button
              ...
              className="rounded-full border border-white/10 bg-[var(--mymedlife-border)]/40 px-3 py-1.5 text-xs font-semibold text-white/52 disabled:cursor-not-allowed"
            >
              {option.label}
            </button>
          ))}
...
function DecisionFact({ body, title }: { body: string; title: string }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-[var(--mymedlife-admin-blue)]/70 p-3">
      <p className="text-xs font-semibold uppercase tracking-[0.16em] text-white/42">
        {title}
      </p>
