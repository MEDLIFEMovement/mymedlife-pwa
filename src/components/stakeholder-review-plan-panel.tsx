    ...
    <section className="rounded-[2rem] border border-[var(--mymedlife-focus-blue)]/20 bg-[var(--mymedlife-focus-blue)]/10 p-5">
      ...
      <div className="mt-5 grid gap-3 lg:grid-cols-3">
        {plan.phases.map((phase) => (
          <article key={phase.id} className="rounded-2xl bg-[var(--mymedlife-border)]/40 p-4">
      ...
      <div className="mt-5 grid gap-3">
        {plan.steps.map((step, index) => (
          <article key={step.id} className="rounded-2xl bg-[var(--mymedlife-border)]/40 p-4">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
              <div>
                ...
                <p className="mt-2 rounded-xl border border-white/10 bg-[var(--mymedlife-admin-blue)]/70 p-3 text-xs leading-5 text-white/52">
              ...
              <div className="flex shrink-0 flex-col gap-2 lg:items-end">
                <Link
                  href={step.route}
                  className="rounded-full bg-[var(--mymedlife-border)] px-3 py-2 text-sm font-semibold text-[var(--mymedlife-on-gold)]"
                >
                  Open {step.route}
                </Link>
