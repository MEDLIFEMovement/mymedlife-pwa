...
export function ProofMetadataVerificationPanel({
  ...
  return (
    <section className="rounded-[2rem] border border-[var(--mymedlife-focus-blue)]/20 bg-[var(--mymedlife-focus-blue)]/10 p-5">
        ...
        <div>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            <Link
              ...
              className="rounded-full border border-white/12 bg-[var(--mymedlife-border)]/40 px-4 py-2 text-sm font-semibold text-white/78"
            >
              Check first write
            </Link>
            {packet.candidateAssignment ? (
              <Link
                ...
                className="rounded-full bg-[var(--mymedlife-border)] px-4 py-2 text-sm font-semibold text-[var(--foreground)]"
              >
      ...
      <section className="mt-5 rounded-[2rem] border border-[var(--mymedlife-focus-blue)]/20 bg-[var(--mymedlife-focus-blue)]/10 p-4">
      ...
      <section className="mt-5 rounded-[2rem] border border-[var(--mymedlife-focus-blue)]/20 bg-[var(--mymedlife-focus-blue)]/10 p-4">
            ...
            <div className="mt-3 grid gap-2">
              {packet.hostedCloseout.reviewSurfaces.map((route) => (
                <Link
                  key={route}
                  href={route}
                  className="rounded-2xl border border-white/10 bg-[var(--mymedlife-admin-blue)]/78 px-3 py-2 font-mono text-xs text-[var(--mymedlife-badge-background)]/80"
                >
                  {route}
                </Link>
              ))}
        ...
        {packet.hostedCloseout.currentObservedEvidence ? (
          <article className="mt-4 rounded-3xl border border-[var(--mymedlife-focus-blue)]/20 bg-[var(--mymedlife-focus-blue)]/10 p-4">
            <p className="text-sm font-semibold text-white">Current hosted evidence</p>
            <p className="mt-2 text-sm leading-6 text-white/64">
              {packet.hostedCloseout.currentObservedEvidence.reviewerNote}
            </p>
            <div className="mt-4 grid gap-3 lg:grid-cols-2">
              <div className="rounded-2xl border border-white/10 bg-[var(--mymedlife-border)]/40 p-3">
                <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[var(--mymedlife-badge-background)]/72">
                  Assignment and evidence item
                </p>
                <p className="mt-2 text-sm font-semibold text-white">
                  {packet.hostedCloseout.currentObservedEvidence.assignmentTitle}
                </p>
                <p className="mt-2 font-mono text-xs text-[var(--mymedlife-badge-background)]/80">
                  assignment={packet.hostedCloseout.currentObservedEvidence.assignmentId}
                </p>
                <p className="mt-2 font-mono text-xs text-[var(--mymedlife-badge-background)]/80">
                  evidence={packet.hostedCloseout.currentObservedEvidence.evidenceItemId}
                </p>
                <p className="mt-2 text-xs leading-5 text-white/58">
                  Status: {packet.hostedCloseout.currentObservedEvidence.assignmentStatus}
                </p>
              </div>
              <div className="rounded-2xl border border-white/10 bg-[var(--mymedlife-border)]/40 p-3">
                <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[var(--mymedlife-badge-background)]/72">
                ...
                <p className="mt-2 font-mono text-xs text-[var(--mymedlife-badge-background)]/80">
                  event={packet.hostedCloseout.currentObservedEvidence.eventId}
                </p>
                <p className="mt-2 font-mono text-xs text-[var(--mymedlife-badge-background)]/80">
                  integration={packet.hostedCloseout.currentObservedEvidence.integrationEventId}
                </p>
                <p className="mt-2 font-mono text-xs text-[var(--mymedlife-badge-background)]/80">
                  audit={packet.hostedCloseout.currentObservedEvidence.auditLogId}
                </p>
                <p className="mt-2 font-mono text-xs text-[var(--mymedlife-badge-background)]/80">
                  outbox={packet.hostedCloseout.currentObservedEvidence.outboxId}
                </p>
              </div>
            </div>
            <p className="mt-4 text-xs leading-5 text-white/58">
              {packet.hostedCloseout.currentObservedEvidence.evidenceSummary}
            </p>
          </article>
        ) : null}
