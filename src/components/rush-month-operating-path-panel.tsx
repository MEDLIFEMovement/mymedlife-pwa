    ...
    <section className="rounded-[2rem] border border-white/10 bg-white/[0.05] p-5">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        <div className="max-w-3xl">
          ...
          {focusStep ? (
            <p className="mt-3 text-xs font-semibold uppercase tracking-[0.2em] text-blue-100/78">
          ...
          {view.steps.map((step, index) => (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#bfdbfe]/46 text-sm font-semibold text-white/72">
                  {index + 1}
                </span>
              ...
              <div className="mt-4 flex items-end justify-between gap-3">
                ...
                <span className="rounded-full border border-white/10 bg-[#bfdbfe]/40 px-2.5 py-1 text-xs font-semibold text-white/64">
                  {stepStateLabel(step)}
                </span>
              </div>
            </article>
      ...
      <p className="mt-4 rounded-2xl border border-white/10 bg-[#bfdbfe]/40 p-3 text-xs leading-5 text-white/58">
        {view.boundaryNote}
      </p>
    </section>
  );
...
function stepCardClassName(step: RushMonthOperatingPathStep): string {
  if (step.isFocus) {
    return "border-blue-300/30 bg-blue-300/10";
  }

  switch (step.stepState) {
    case "complete":
      return "border-blue-300/20 bg-blue-300/10";
    case "current":
      return "border-blue-300/30 bg-blue-300/10";
    case "upcoming":
      return "border-white/10 bg-[#bfdbfe]/40";
  }
}
