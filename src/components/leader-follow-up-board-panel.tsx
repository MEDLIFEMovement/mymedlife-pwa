...
export function LeaderFollowUpBoardPanel({ board }: LeaderFollowUpBoardPanelProps) {
  ...
  return (
    <section className="rounded-[2rem] border border-white/10 bg-white/[0.05] p-5">
      ...
      {board.rows.length === 0 ? (
        <p className="mt-4 rounded-2xl bg-black/20 p-3 text-sm leading-6 text-white/64">
          {board.emptyMessage}
        </p>
      ) : (
        <div className="mt-5 grid gap-3">
          {board.rows.map((row) => (
            <article key={row.assignmentId} className="rounded-2xl bg-black/20 p-4">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                ...
                <Link
                  href={row.nextHref}
                  className="shrink-0 rounded-full border border-emerald-300/20 bg-emerald-300/10 px-4 py-2 text-sm font-semibold text-emerald-100"
                >
                  Open next step
                </Link>
              </div>
