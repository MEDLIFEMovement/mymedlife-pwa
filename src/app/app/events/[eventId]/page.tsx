...
type RsvpCancelControlProps = Readonly<{
  event: NonNullable<ReturnType<typeof getMemberLaunchLaneEventRowById>>;
  source?: string;
  profileSource?: string;
  campaign?: string;
...
type EventLoopResultBannerProps = Readonly<{
  resultState: EventLoopResultState;
  className?: string;
}>;
...
function EventLoopResultBanner({
  resultState,
  className = "",
}: EventLoopResultBannerProps) {
  if (!resultState) {
    return null;
  }

  const classes = getEventLoopResultBannerClasses(resultState.tone);
...
function getEventLoopResultBannerClasses(tone: NonNullable<EventLoopResultState>["tone"]) {
  if (tone === "success") {
  ...
  return "border-[#bfdbfe] bg-[#eff6ff] text-[#1b4b8e]";
}

type EventLoopActionFormProps = Readonly<{
