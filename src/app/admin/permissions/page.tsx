...
export default async function AdminPermissionsPage({
  ...
  const workspace = getAdminPermissionsWorkspace(actor, undefined, resolvedSearchParams);

  return (
    <AppShell actor={actor}>
      <DataSourceNotice source={data.source} />
      <AdminBackendLaneNav current="permissions" />
      ...
      {!workspace.canReadWorkspace ? (
        ...
        <>
          <section className="rounded-[2rem] border border-white/12 bg-[#1c1736] p-5">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              ...
              <Link
                ...
                className="w-fit rounded-full bg-violet-200 px-4 py-2 text-sm font-semibold text-[#1c1736]"
              >
              ...
              <div className="flex flex-wrap gap-2">
                {workspace.sectionOptions.map((option) => (
                  <Link
                    ...
                    className={
                      option.selected
                        ? "rounded-full bg-violet-200 px-3 py-1.5 text-sm font-semibold text-[#1c1736]"
          ...
          <section className="rounded-[2rem] border border-white/10 bg-[#071d1a]/90 p-5">
            ...
            {workspace.focusedSection.selectedCard ? (
              <section className="mt-4 rounded-[1.5rem] border border-violet-200/18 bg-violet-200/10 p-4">
                <p className="text-xs font-semibold uppercase tracking-[0.2em] text-violet-100/80">
                  Selected in registry
                </p>
                <div className="mt-3 flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  <div className="max-w-3xl">
                    <p className="text-[0.68rem] font-semibold uppercase tracking-[0.18em] text-violet-100/72">
                    ...
                    <p className="mt-3 text-sm leading-6 text-violet-100/78">
            ...
            <div className="mt-4 grid gap-3 lg:grid-cols-2">
              {workspace.focusedSection.cards.map((card) => (
                <article
                  ...
                  className={[
                    ...
                    workspace.focusedSection.selectedKey === card.key
                      ? "border-violet-200/24 bg-violet-200/10"
                ...
                >
                  ...
                  <p className="mt-3 text-sm leading-6 text-violet-100/72">
                    {card.footer}
                  </p>
          ...
          <section className="rounded-[2rem] border border-violet-200/20 bg-violet-200/10 p-5">
