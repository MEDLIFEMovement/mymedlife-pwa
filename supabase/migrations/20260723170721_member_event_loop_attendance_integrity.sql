...
on app.points_events (chapter_event_id, awarded_to_user_id)
where chapter_event_id is not null
  and reason = 'Attendance confirmed through the myMEDLIFE member event loop.';
...
declare
  ...
  attendance_reason constant text := 'Attendance confirmed through the myMEDLIFE member event loop.';
  legacy_attendance_reason constant text := 'Attendance confirmed through the production-safe TEST event loop.';
...
begin
  ...
  where chapter_event_id = chapter_event_uuid
    ...
    and reason in (attendance_reason, legacy_attendance_reason)
  ...
  if existing_points.id is not null then
    ...
    return query select
      ...
      target_event.id,
      existing_points.points_delta,
      coalesce(next_attendance_count, 0);
    return;
  end if;
  ...
  if inserted_points_id is null then
    select *
    into existing_points
    from app.points_events
    where chapter_event_id = target_event.id
      and awarded_to_user_id = actor_uuid
      and reason = attendance_reason
    order by created_at asc, id asc
    limit 1;
    ...
    return query select
      'already_checked_in'::text,
      target_event.id,
      coalesce(existing_points.points_delta, 20),
      coalesce(next_attendance_count, 0);
    return;
  end if;
