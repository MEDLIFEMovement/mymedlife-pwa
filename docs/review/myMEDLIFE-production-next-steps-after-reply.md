...
This note explains what Codex will do after the approval thread gets a reply.
It keeps the next step simple so reviewers do not have to guess how their answer
will be used.

## If the reply is `approved as written`
...
4. Leave broad writes, non-approved Luma behavior, live external sends, and
   production launch disabled until the hosted proof is visible and the
   remaining launch gates are clear.

## If the reply changes one or more fields

Codex will:

1. Update only the fields that were explicitly changed.
2. Keep all other default values unchanged.
3. Re-issue the reviewer-facing summary if the change affects the launch
   packet or the send-ready email.
4. Keep the packet in review-only posture until the revised answers are
   reflected everywhere.

## What Codex Will Not Do From A Reply Alone
...
The primary approver has already approved the defaults for:
...
This is a handoff note, not a launch decision.
