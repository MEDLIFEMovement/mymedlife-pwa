...
| Luma event loop | The first event loop is Luma-backed: event create/update, RSVP writeback, attendance import, points and leaderboard readback, with no n8n or broad external sends. | Hosted staging evidence still needs final reviewer confirmation. | Events / DS |
