...
- The `/admin/luma-live-pilot` cross-role proof should explicitly show:
  - member leaderboard readback at `/rush-month/leaderboard`
  - leader/staff chapter leaderboard readback at `/chapter?view=leaderboard`
  - `Points + leaderboard` on the staff proof card
  - `Leaderboard posture` on the admin proof card
- `/admin/feature-flags` and `/admin/theme` should show the persisted
  Supabase-backed state when the reviewer session is signed in, including the
  audit row count and the `Supabase-backed` badge instead of the persistence
  warning.
