...
| Staff/Admin panel | `docs/figma-code/raw/staff-command-center/App.tsx`, `AdminRoleGate`, `AdminPanel`; `mock-data.tsx` | `/staff?view=admin` and/or `/admin` | Staff/Admin backend | Admin | `src/components/figma-staff-command-center.tsx`; existing `/admin` backend | DS/Super Admin gate and admin health/outbox/audit panel | Staff shell now has System Health preview; `/admin` backend remains protected and separate. |
| SLT Prep | No current raw Figma export in this package | `/app/slt-prep` and/or `/slt-prep` | SLT Prep | SLT Prep | `src/app/slt-prep/page.tsx` | Traveler readiness workflow | Exact local Figma code missing; should show explicit missing-Figma placeholder if source remains unavailable. |
