...
| Admin vertical nav: MCP Connections | hidden from primary launch menu | `figma-admin-panel.tsx` | DS Admin / Super Admin | Keep MCP out of the launch-critical menu | Removed from primary admin nav; MCP Analytics remains visibly disabled in Disabled Modules | future audited MCP policy service | DS Admin / Super Admin | MCP admin flags | `disabled_hidden` |
