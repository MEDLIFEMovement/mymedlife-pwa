...
| Admin vertical nav: MCP Connections | `/admin` | `figma-admin-panel.tsx` | DS Admin / Super Admin | Review read-only MCP posture | Local Figma shell screen state; write access remains disabled | future audited MCP policy service | DS Admin / Super Admin | MCP admin flags | `placeholder_blocked` |
...
| User chip / logout icon | `/admin` | `figma-admin-panel.tsx` | DS Admin / Super Admin | Account/session affordance | Visual Figma shell control only | app account menu / sign-out action | signed-in admin | none | `placeholder_blocked` |
